  ...
  ast.walkAtRules('theme', (rule) => {
    rule.walkDecls((decl) => {
      if (decl.prop.startsWith('--color-') && !decl.prop.includes('*')) {
        ...
        if (!colorName.match(/^(brand|neutral|destructive|warning|success|info)-\d+$/)) {
...
function generateTypeDefinitions(utilities, themeColors) {
  ...
  const colorUtilities = themeColors.flatMap(color => 
...
function generateDocSection(utilities) {
  ...
  const typography = utilities.filter(u => u.name.startsWith('text-') || u.name.startsWith('typography-'));
  const backgrounds = utilities.filter(u => u.name.startsWith('bg-'));
  const borders = utilities.filter(u => u.name.startsWith('border-'));
  const other = utilities.filter(u => 
...
function updateClaudeMd(newContent) {
  ...
  if (startIndex === -1) {
    console.error('Could not find "## Available Utility Classes" section in CLAUDE.md');
    console.error('Please ensure CLAUDE.md contains this section for auto-generation to work');
    return;
  }
...
async function main() {
  ...
  try {
    ...
    console.log(`   Written to ${path.relative(process.cwd(), UTILITIES_TYPES)}`);
    ...
    console.log('   import type { MoonshineClasses } from "@speakeasy-api/moonshine/types/utilities"');
