...
export function PullRequestLink({
  ...
  return (
    <a
      className={cn(
        "inline-flex flex-row items-center gap-1 rounded-lg border px-1.5 py-1 text-sm transition-colors duration-500 hover:bg-muted-foreground/5",
        className,
      )}
      href={href}
      target={target}
    >
      ...
      <span className="text-xs font-semibold text-body">{`#${num}`}</span>
    </a>
  );
}
