...
const SegmentedButtonBase = ({ children, className }: SegmentedButtonProps) => {
  ...
  return (
    <div
      className={cn(
        "relative inline-flex flex-row items-center overflow-hidden rounded-full border border-neutral-softest shadow-xs backdrop-blur-md",
        className,
      )}
    >
      {enhancedChildren}
...
const SegmentedButtonItem = ({
  ...
  return (
    <motion.button
      className="relative flex items-center rounded-full px-5 py-1 text-codeline-xs uppercase hover:text-highlight"
      {...props}
    >
      {highlighted && (
        <motion.span
